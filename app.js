...
async function loadNews(){
  try{
    ...
    const data=await res.json();const official=data.sources?.packers||[];const acme=data.sources?.acme||[];const espn=data.sources?.espn||[];
    const packers=[...official,...acme].sort(sortNewest);
    ...
    if(packers[0]){const top=packers[0];$('#heroTitle').textContent=top.title;$('#heroSummary').textContent=top.summary||`Read the latest from ${top.source}.`;$('#heroSource').textContent=top.source.toUpperCase();$('#heroTime').textContent=relativeTime(top.publishedAt);$('#heroLink').href=top.url;$('#heroLink').target='_blank';$('#heroLink').rel='noopener';}
    const acmeStatus=$('#acmeStatus');if(acmeStatus)acmeStatus.textContent=acme.length?'Acme Packing Company ✓':'Acme Packing Company';
    const sourceNote=$('#sourceNote');if(sourceNote)sourceNote.textContent=acme.length?'Packers.com, Acme and ESPN are live now. The remaining sources will be added only when we have a reliable feed.':'Packers.com and ESPN are live now. Acme is being tested automatically.';
